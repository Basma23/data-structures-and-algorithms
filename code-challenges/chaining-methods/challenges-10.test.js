...
const createServer = () => {
  ...
  app.get('/aboutme', (request,response) =>{
    let aboutme = 'Basma Nizar MohmmdEid, ASAC student, Front-end developer';
    response.status(200).json(aboutme);
  });
  app.get('/favoritefoods', (request, response) =>{
    let myFavoriteFood = ['Yallengy', 'Beryani', 'Kabsa', 'Maqloba'];
    response.status(200).json(myFavoriteFood);
  });
...
const count = (target, input) => {
  ...
  let num = [];
  let count = 0;
  let all = input.map(element =>{
    element.map(value =>{
      num.push(value);
    });
    count = num .reduce((accumulator, value) =>{
      if(target === value){
        accumulator++;
      }
      return accumulator
